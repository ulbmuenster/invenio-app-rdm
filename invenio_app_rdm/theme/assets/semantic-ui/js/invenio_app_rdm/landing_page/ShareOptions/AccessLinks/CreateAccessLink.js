...
checkIfIsAccessLinksExpirationRequired  = () => {
    return document.getElementById("deposit-share-btn-require-link-expiration")?.dataset?.shareBtnRequireLinkExpiration ||
      document.getElementById("detail-share-btn-require-link-expiration")?.dataset?.shareBtnRequireLinkExpiration ||
      false;
}

  render() {
    const { permission, expiresAt, description } = this.state;
    const { hasError, handleCreation, loading, dropdownOptions } = this.props;
    const isAccessLinksExpirationRequired = this.checkIfIsAccessLinksExpirationRequired();
    const linkExpirationDateHelptext = isAccessLinksExpirationRequired === "true" ?
